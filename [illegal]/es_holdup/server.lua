...
--[[ 	["paleto_twentyfourseven"] = {
		...
		reward = math.random(15500, 25000),
...
--[[ 	["bar_one"] = {
	...
	["littleseoul_twentyfourseven"] = {
		...
		reward = math.random(8500, 15500),
		nameofstore = "Twenty Four Seven. (Little Seoul)",
		lastrobbed = 0
	},
	["barbareno_twentyfourseven"] = {
		position = { ['x'] = -3249.71508789063, ['y'] = 1004.11895751953, ['z'] = 79.4371795654297 },
		reward = math.random(5500, 10500),
		nameofstore = "Twenty Four Seven. (barbareno)",
		lastrobbed = 0
	},
	["palomino_twentyfourseven"] = {
		position = { ['x'] = 2549.69653320313, ['y'] = 384.809051513672, ['z'] = 108.622932434082 },
		reward = math.random(5500, 10500),
		nameofstore = "Twenty Four Seven. (palomino)",
		lastrobbed = 0
	},
}
...
AddEventHandler('es_holdup:rob', function(robb,nbPolice,source)
	if nbPolice >= 2 then
		if stores[robb] then
			...
			if (os.time() - store.lastrobbed) < 900 and store.lastrobbed ~= 0 then
				TriggerClientEvent('chatMessage', source, 'BRAQUAGE', {255, 0, 0}, "Ce store à déja été cambriolé: ^2" .. (900 - (os.time() - store.lastrobbed)) .. "^0 secondes.")
				return
			end
			TriggerClientEvent('chatMessage', -1, 'NEWS', {255, 0, 0}, "Braquage en cours ^2" .. store.nameofstore)
			TriggerClientEvent('chatMessage', source, 'SYSTEM', {255, 0, 0}, "vous avez commncer un braquage à ^2" .. store.nameofstore .. "^0, ne vous éloignez pas du magasin")
			...
			SetTimeout(180000, function()
				if(robbers[savedSource])then
					local reward = math.floor(store.reward * (1.2^nbPolice))
