...
--[[ 	["bar_one"] = {
	...
	["barbareno_twentyfourseven"] = {
		position = { ['x'] = -3249.71508789063, ['y'] = 1004.11895751953, ['z'] = 12.830714225769 },
...
AddEventHandler('es_holdup:toofarlocal', function(robb)
	holdingup = false
	TriggerEvent('chatMessage', 'SYSTEM', {255, 0, 0}, "Braquage annulé, tu ne recevra rien !")
	robbingName = ""
	secondsRemaining = 0
	incircle = false
end)
...
AddEventHandler('es_holdup:robberycomplete', function(reward)
	holdingup = false
	TriggerEvent('chatMessage', 'SYSTEM', {255, 0, 0}, "Bravo ! tu as gagné: ^2" .. reward)
	store = ""
	secondsRemaining = 0
	incircle = false
end)
				...
				if not holdingup then
					...
					if(Vdist(pos.x, pos.y, pos.z, pos2.x, pos2.y, pos2.z) < 1.0)then
						if (incircle == false) then
							DisplayHelpText("~INPUT_CONTEXT~ pour cambrioler ~b~" .. v.nameofstore .. "~w~ la police va etre prevenue!")
						end
						incircle = true
