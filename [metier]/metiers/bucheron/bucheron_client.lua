...
AddEventHandler('bucheron:drawGetBois', function (qteBois)
	...
	if qteBois < 90 then
		TriggerEvent('player:receiveItem',19, 2)
		ClearPrints()
		SetTextEntry_2("STRING")
...
AddEventHandler('bucheron:drawGetPlanche', function(qteBois, qtePlanche)
	...
	if qtePlanche < 90 and qteBois > 1 then
		TriggerEvent('player:looseItem',19, 2)
		TriggerEvent('player:receiveItem',20, 2)
		ClearPrints()
		SetTextEntry_2("STRING")
...
AddEventHandler('bucheron:drawSellPlanche', function (qte)
	...
	if qte > 1 then
		TriggerEvent('player:looseItem',20, 2)
