...
function GetServiceFermier()
	...
	AddEventHandler('fermier:drawGetBle', function (qteBle)
		...
		if qteBle < 90 then
			TriggerEvent('player:receiveItem',10, 2)
			ClearPrints()
			SetTextEntry_2("STRING")
	...
	AddEventHandler('fermier:drawGetFarine', function(qteBle, qteFarine)
		...
		if qteFarine < 90 and qteBle > 1 then
			TriggerEvent('player:looseItem',10, 2)
			TriggerEvent('player:receiveItem',5, 2)
			ClearPrints()
			SetTextEntry_2("STRING")
			AddTextComponentString("~g~Vous déposez votre blé et récupérer de la farine.")
			DrawSubtitleTimed(4500, 1)
		else
			ClearPrints()
			SetTextEntry_2("STRING")
	...
	AddEventHandler('fermier:drawSellFarine', function (qte)
		...
		if qte > 1 then
			TriggerEvent('player:looseItem',5, 2)
